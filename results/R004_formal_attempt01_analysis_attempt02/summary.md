...
## Complete saved-checkpoint history

- `categorical_reference_decay`: ever observed exact 4, later inexact at the endpoint 1, never observed exact 12.
- `truth_reference_decay`: ever observed exact 2, later inexact at the endpoint 0, never observed exact 14.
- `categorical_no_decay`: ever observed exact 8, later inexact at the endpoint 0, never observed exact 8.
- `truth_no_decay`: ever observed exact 4, later inexact at the endpoint 1, never observed exact 12.
...
The fixed profiles account for 512 labeled modes and 33792 labeled trajectories. Observed phase dependence occurs in 86 modes, with explicit initialization counterexamples in 86; 144 universal tests certify all-state visible settling, 108 certify persistent visible failure, and 260 are inconclusive. Across labeled starts, 12752 have an orbit-certified correct onset, 3606 enter mixed-phase cycles, 16054 enter cycles with no correct phase, and 1380 remain unresolved at the cap. Exactly 76 modes are correct at tick 20 for all 66 fixed starts.
